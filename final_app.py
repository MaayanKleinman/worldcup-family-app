...
st.markdown(
    "<h1 style='text-align: center; color: #e61d25;'>🏆 מונדיאל 2026 - המשפחה 🏆</h1>",
    unsafe_allow_html=True,
)
...
# חשוב: עוצרים רק אם הייתה תקלה אמיתית בשליפת המשחקים.
# אם השרת מחזיר [] - האפליקציה ממשיכה ומציגה שאין משחקים פתוחים.
if all_wc_matches is None:
    st.error(
        "❌ תקלה קריטית בשליפת המשחקים משרתי הנתונים! לא ניתן להמשיך. "
        "אנא רעננו את העמוד או נסו מאוחר יותר."
    )
    st.stop()

if all_wc_standings is None:
    st.warning(
        "⚠️ תקלה זמנית בטעינת טבלאות הבתים. ניתן להמשיך למלא ניחושים יומיים כרגיל! "
        "(ייתכן שבונוס הבתים בטבלה לא מעודכן כרגע)."
    )
    all_wc_standings = []

teams_a = ["מקסיקו 🇲🇽", "דרום אפריקה 🇿🇦", "קוריאה הדרומית 🇰🇷", "צ'כיה 🇨🇿"]
teams_b = ["קנדה 🇨🇦", "בוסניה והרצגובינה 🇧🇦", "קטאר 🇶🇦", "שווייץ 🇨🇭"]
teams_c = ["ברזיל 🇧🇷", "מרוקו 🇲🇦", "האיטי 🇭🇹", "סקוטלנד 🏴"]
...
teams_l = ["אנגליה 🏴", "קרואטיה 🇭🇷", "גאנה 🇬🇭", "פנמה 🇵🇦"]
...
with tab1:
    st.markdown(
        ...
        "border-right: 5px solid #e61d25; color: #b30000; font-weight: bold;'>"
        "⚠️ שימו לב: הניחוש תקף ל-90 דקות משחק בלבד! "
        "(כולל תוספת זמן פציעות, לא כולל הארכות ופנדלים)</div>",
        unsafe_allow_html=True,
    )
    st.write("")
    ...
    for i in range(3):
        ...
        if daily_events:
            st.markdown(
                f"### 📅 משחקי {date_label} "
                f"({current_loop_date_str.split('-')[2]}/{current_loop_date_str.split('-')[1]}):"
            )
            ...
            for match in daily_events:
                ...
                if match.get("status") == "FINISHED":
                    ...
                    lock_text = (
                        f"🏁 המשחק הסתיים! תוצאת אמת: "
                        f"{home_heb} {score_home} - {score_away} {away_heb}"
                    )
                elif is_locked:
            ...
            if day_has_open:
                if st.button(f"💾 שמור את ניחושי {date_label}", key=f"save_{current_loop_date_str}_{username}"):
                    ...
                    joker_in_short_day = any(
                        d["joker"] and d["total_games_day"] < 3 for d in day_inputs.values()
                    )
                    ...
                    if joker_count > 1:
                        ...
                        st.error(
                            "⚠️ לא ניתן להשתמש בג'וקר ביום זה! "
                            "חוק הג'וקר תקף רק לימים בהם משוחקים 3 משחקים ומעלה."
                        )
                    else:
                        if sheet:
                            try:
                                ...
                                for m_id, data in day_inputs.items():
                                    ...
                                    for idx, row in enumerate(all_rows):
                                        if (
                                            len(row) > 2
                                            and row[1].strip() == username.strip()
                                            and row[2].strip() == str(m_id).strip()
                                        ):
                                ...
                                st.success(
                                    f"🎉 כל הכבוד {username}! "
                                    f"הניחושים שלך ליום {date_label} נשמרו בהצלחה!"
                                )
                                st.rerun()
                            except Exception as e:
...
with tab2:
    ...
    champ = st.selectbox(
        "🥇 מי תהיה האלופה ותניף את הגביע בסוף הטורניר?",
        ALL_48_TEAMS,
        index=def_champ,
        disabled=is_tournament_started,
        key=f"champ_{username}",
    )
    st.write("---")
...
with tab3:
    ...
    scores_table = {
        member: {"משחקים": 0, "בונוס טורניר": 0, "סך הכל": 0}
        ...
        try:
            ...
            for m in all_wc_matches:
                if m.get("status") == "FINISHED":
                    ...
                    if full_time.get("home") is not None and full_time.get("away") is not None:
                        actual_results[str(m.get("id"))] = {
                            "home": int(full_time.get("home")),
                            "away": int(full_time.get("away")),
                        }

                    if m.get("stage") == "FINAL":
                        winner_code = m.get("score", {}).get("winner")
                        if winner_code == "HOME_TEAM":
                            actual_champion = clean_string(
                                get_team_name_heb(m.get("homeTeam", {}).get("name"))
                            )
                        elif winner_code == "AWAY_TEAM":
                            actual_champion = clean_string(
                                get_team_name_heb(m.get("awayTeam", {}).get("name"))
                            )
            ...
            for group_data in all_wc_standings:
                ...
                if len(g_table) > 1:
                    ...
                    actual_group_runners_up[g_name] = clean_string(
                        get_team_name_heb(second_team_en)
                    )
                ...
                for row in user_guesses:
                    ...
                    if g_match_id in actual_results:
                        ...
                        if g_home == real["home"] and g_away == real["away"]:
                            ...
                            or (g_home < g_away and real["home"] < real["away"])
                            or (g_home == g_away and real["home"] == real["away"])
                        ):
                            match_points = 1
            ...
            for member in scores_table:
                scores_table[member]["סך הכל"] = (
                    scores_table[member]["משחקים"]
                    + scores_table[member]["בונוס טורניר"]
                )
    ...
    formatted_data = [
        {
            "משתמש": m,
            "⚽ נקודות משחקים": d["משחקים"],
            "🏆 בונוס טורניר": d["בונוס טורניר"],
            "🔥 סך הכל": d["סך הכל"],
        }
        for m, d in scores_table.items()
    ]
